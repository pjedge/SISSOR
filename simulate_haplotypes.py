    ...
    for i in range(0, num_reads):
        ...
        for j in range(1,len(frag.seq)):
            if random.random() < alpha:
                switchlist.append(frag_snp_bak+j)
        ...
        lines.append((firstpos, frag, switchlist))
...
#CHROM	POS	ID	REF	ALT	QUAL	FILTER	INFO	FORMAT	SIM_INDIVIDUAL'''.format(ref_length)
    ...
    with open(frag_file, 'w') as opf:
        for i,(firstpos, frag, switches) in enumerate(lines):
            frag.frag_id = 'FRAG{}'.format(i)
            line = format_fragment(frag)
            for switchpos in switches:
                print("SWITCH in {} at pos {}".format(frag.frag_id,switchpos))
            print(line, file=opf)
