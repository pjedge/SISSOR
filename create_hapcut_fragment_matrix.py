...
def parse_args():
    ...
    parser.add_argument('variant_vcf_file', nargs='?', type = str, help='vcf file containing reliable variant calls to be used with HapCUT')
...
def create_sissor_fragments(sissor_vcf_file, bed_file, variant_vcf_file, pileup_file, output_fragment_file):
    ...
    if pileup_file != None:
        with open(pileup_file, 'r') as infile:
            ...
            for line in infile:
                ...
                base_calls = [c for c in el[8] if c in ['.',',','A','T','G','C','a','t','g','c']]
                base_quals = list(el[9])

                assert (len(base_calls) == len(base_quals))
